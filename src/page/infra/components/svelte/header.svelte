<script lang="ts">
	 ...
	 */
	function openModal(modal: Modal.Modal): () => void {
		return () => med.dispatch(SCREEN_ACTIONS.openModal, { modal })
	}

	$: inGame = Route.Game.is($pageState.route)
</script>

<header class="status-bar vimdoku-header monospace">
	<section>
		<button class="status-icon icon-dialog" on:click={openModal(new Modal.Cmd())}><Icon id="cmd" /></button>
	</section>
	...
	<section>
		<button
			...
			on:click={openModal(inGame ? new Modal.Pause() : new Modal.Pref(Modal.PrefType.edit))}
