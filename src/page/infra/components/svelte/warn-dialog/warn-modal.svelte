<script lang="ts">
	...
	$: type = Modal.Warn.is($pageState.modal) ? $pageState.modal.type : type

	/** Game save and Exit, click handler. */
	function saveAndExitHandler() {
		med.dispatch(SUDOKU_ACTIONS.save).dispatch(SCREEN_ACTIONS.back).dispatch(SCREEN_ACTIONS.back)
	}
</script>

<Dialog type="modal" show={Modal.Warn.is($pageState.modal)}>
